...
- **НОВИНКА**: Перемикач CPU/GPU в конфігурації

### 5. Перемикач CPU/GPU
Тепер можна вибрати пристрій для обчислень через конфігурацію:

```ini
[MAIN]
sDevice = auto    # auto, cpu, gpu
```

- `auto` - автоматичний вибір (GPU якщо доступний, інакше CPU)
- `cpu` - примусово використовувати CPU
- `gpu` - примусово використовувати GPU (помилка якщо GPU недоступний)
